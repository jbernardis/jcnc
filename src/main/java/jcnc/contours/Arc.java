...
public class Arc extends CncDialog {
	Text tDiameter;
	Text tSAngle;
	Text tEAngle;
	Text tCenterX;
	...
	public Arc(Shell parent, CncProperties props) {
		super(parent, props);
		objectName = "Arc Contour " + seqNo;
	}
	...
	private void createContents() {
		...
		addLabel("Start Angle: ");
		tSAngle = addDoubleText(0.0F);
		
		addLabel("End Angle: ");
		tEAngle = addDoubleText(45.0F);
		...
		bCutRadii = new Button(shell, SWT.CHECK);
		bCutRadii.setText("Cut Radii");
		bCutRadii.addListener(SWT.Selection, new Listener() {
			public void handleEvent(Event event) {
				cutRadii = bCutRadii.getSelection();
				setModified(true);
			}
		});
		gd = new GridData();
		gd.horizontalSpan = 4;
		gd.horizontalAlignment = SWT.CENTER;
		bCutRadii.setLayoutData(gd);
		cutRadii = true;
		bCutRadii.setSelection(cutRadii);
	...
	private Group addToolMovement() {
		...
		String[] tags = {"On Arc", "Outside", "Inside"};
	...
	private void generateGCode() {
		...
		double sangle = Double.valueOf(tSAngle.getText());
		double eangle = Double.valueOf(tEAngle.getText());
		...
		tGCode.append("(Arc   Diameter: " + fmtDbl(diameter) + " ");
		tGCode.append("center at " + fmtDbl(cx) + ", " + fmtDbl(cy) + " ");
		tGCode.append("depth from " + fmtDbl(sz) + " to -" + fmtDbl(depth) + ")\n");
		tGCode.append("(Start/End angles: " + fmtDbl(sangle) + "/" + fmtDbl(eangle) + ")\n");;
		...
		tGCode.append("(Pocket: " + pocket + "   Cut Radii: " + cutRadii + ")\n");
		...
		if (toolMovement.equals("Inside"))
			diameter = diameter - tooldiam;
		else if (toolMovement.equals("Outside"))
			diameter = diameter + tooldiam;
		
		boolean doCutRadii = cutRadii;
		if (pocket)
			doCutRadii = true;
		

		double rad = diameter / 2.0;
		double cosstart = Math.cos(Math.toRadians(sangle));
		double sinstart = Math.sin(Math.toRadians(sangle));
		double cosend = Math.cos(Math.toRadians(eangle));
		double sinend = Math.sin(Math.toRadians(eangle));
		double startxoff = rad * cosstart;
		double startyoff = rad * sinstart;
		double endxoff = rad * cosend;
		double endyoff = rad * sinend;
		...
		for (int i=0; i<passes; i++) {
			...
			if (pocket) {
				...
				rad = tooldiam * stepover;
				while (rad < diameter/2.0) {
					double pktstxoff = rad * cosstart;
					double pktstyoff = rad * sinstart;
					double pktendxoff = rad * cosend;
					double pktendyoff = rad * sinend;
					
					tGCode.append(moveXY(cx+pktstxoff, cy+pktstyoff, "G0", stXYRapid));
					tGCode.append(moveZ(cz, "G1", stZNormal));
					
					tGCode.append(cmd + IJTerm("I", -pktstxoff) + IJTerm("J", -pktstyoff) + 
							" X" + fmtDbl(cx+pktendxoff) + " Y" + fmtDbl(cy+pktendyoff) + stXYNormal + "\n");
					tGCode.append(moveZ(safez,  "G0",  stZRapid));
					
					rad = rad + tooldiam * stepover;
				}
			...
			if (doCutRadii) {
				tGCode.append(moveZ(cz, "G1", stZNormal));
				tGCode.append(moveXY(cx + startxoff, cy + startyoff, "G1", stXYNormal));
			}
			else {
				tGCode.append(moveXY(cx + startxoff, cy + startyoff, "G0", stXYRapid));
				tGCode.append(moveZ(cz, "G1", stZNormal));
			}
	
			tGCode.append(cmd + IJTerm("I", -startxoff) + IJTerm("J", -startyoff) + " X" + fmtDbl(cx + endxoff) +
					" Y" + fmtDbl(cy + endyoff) + stXYNormal + "\n");
	...
	private String IJTerm(String tag, double val) {
		if (val == 0.0)
			return "";
		
		return " " + tag + fmtDbl(val);
	}
	
	private void JSONInit() {
        JSONText.clear();
        JSONText.put("diameter",  tDiameter);
        JSONText.put("startangle",  tSAngle);
        JSONText.put("endangle",  tEAngle);
	...
	private void deserialize() {
        ...
        cutRadii = (boolean) objData.get("cutradii");
        bCutRadii.setSelection(cutRadii);
